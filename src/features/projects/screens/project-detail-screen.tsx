...
export default function ProjectDetailScreen({
  ...
  const handleIssueSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIssueFormData({ title: '', description: '', type: null, priority: null })
  }
  ...
  const handleConfirmVisibilityChange = () => {
    if (pendingVisibility) {
      setSettingsFormData(prev => ({ ...prev, visibility: pendingVisibility }))
    }
  ...
  const handleMemberDialogOpen = (open: boolean) => {
    ...
    if (open && members.length === 0) {
      ...
      setTimeout(() => {
        ...
        const { MOCK_PROJECT_MEMBERS, MOCK_PROJECT_INVITATIONS } = require('@/mocks')
        setMembers(MOCK_PROJECT_MEMBERS.filter((m: any) => m.projectId === project.id))
        setPendingInvitations(MOCK_PROJECT_INVITATIONS.filter((i: any) => i.projectId === project.id && i.status === 'pending'))
    ...
    <div className="mx-auto max-w-7xl px-6 py-8 lg:px-8">
      ...
      <ProjectDetailHeader
        ...
        <IssuesCreateDialog
          ...
          onTitleChange={(value) => setIssueFormData(prev => ({ ...prev, title: value }))}
          onDescriptionChange={(value) => setIssueFormData(prev => ({ ...prev, description: value }))}
          onTypeChange={(value) => setIssueFormData(prev => ({ ...prev, type: value }))}
          onPriorityChange={(value) => setIssueFormData(prev => ({ ...prev, priority: value }))}
      ...
      renderLeaveButton={(trigger) => (
        <ProjectLeaveButton
          projectName={project.name}
          userRole={(userRole || 'member') as 'owner' | 'editor' | 'member' | 'viewer'}
          isLeaving={isLeaving}
          error={leaveError}
          onLeave={handleLeave}
        >
