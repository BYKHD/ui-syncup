...
import { cancelAccessRequest, createAccessRequest, type AccessRequest } from '../../api'
...
describe('AccessRequestPanel', () => {
  ...
  it('transitions from request form to pending after create succeeds', async () => {
    ...
    await user.type(screen.getByPlaceholderText('Add a note (optional)'), 'Please add me')
  ...
  it('transitions from pending to request form after cancel succeeds', async () => {
    ...
    mockCancelAccessRequest.mockResolvedValueOnce({ ...existingRequest, status: 'cancelled' })
