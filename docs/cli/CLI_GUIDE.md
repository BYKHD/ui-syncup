...
- **(Phase 2)** Interactive credential setup for external services
...
During `init`, you can optionally configure:

```text
? Configure external services? (Press Enter to skip)
  > Skip for now (use defaults)
    Email (Resend)
    OAuth (Google, Microsoft)
```

| Service | Default Behavior | When Configured |
|---------|-----------------|----------------|
| Email | Console logging | Actual delivery via Resend |
| Google OAuth | Disabled (email/password only) | Social login enabled |
| Microsoft OAuth | Disabled | Social login enabled |
...
### Purpose
Diagnose environment and configuration issues. Provides comprehensive validation of your setup.

### Usage
```bash
ui-syncup doctor
ui-syncup doctor --fix        # Auto-fix common issues
ui-syncup doctor --json       # Output as JSON for scripting
```
...
|----------|------------------|
| **System** | Bun, Docker, Supabase CLI availability |
| **Database** | Connection, schema version, migration status |
| **Storage** | Directory permissions, provider access |
| **Auth** | OAuth provider configuration status |
...
Optional Services:
  ⚠ Email not configured (console fallback active)
  ⚠ Google OAuth not configured (email/password only)
  ✔ Microsoft OAuth configured

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Status: Ready (2 optional services not configured)
```
...
⚠ Optional (not blocking):
  - RESEND_API_KEY not set (emails will log to console)
