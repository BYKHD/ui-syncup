...
> [!warning] **`next/image` re-fetches `src` whenever your `onError` identity changes.** `ImageElement`'s internal ref callback (`next/dist/client/image-component.js`) lists `onError` in its `useCallback` deps and does `img.src = img.src` on re-attach — a deliberate workaround for errors lost before hydration. An inline `onError`/`onLoad` in the parent therefore forces a **full network re-fetch of the image on every render**. Keep `handleImageLoad`/`handleImageError` in `centered-canvas-view.tsx` wrapped in `useCallback`. With a `blob:` src this is not just wasted bandwidth: it is fatal once the URL has been revoked.
...
- **Create-dialog canvas "Failed to load image"** (2026-08-10): pasting the as-is image, then the to-be image, then typing in title/description blanked the as-is canvas into the error panel. Two defects chained. (1) `issues-create-dialog.tsx`'s "cleanup blob URLs **on unmount**" effect listed `[asIsPreview, toBePreview]` as deps — React runs a cleanup on every dep change, so setting the second image revoked the **first, still-displayed** blob URL (symmetric in both directions). Fix: read the previews through a ref so the cleanup effect can take `[]`. (2) The revoked URL stayed visible until something re-fetched it — that was `next/image`'s ref self-assign (gotcha above), fired by the unstable `handleImageError` identity on the first keystroke. Fix: `useCallback` both handlers, which also removes a per-render image re-fetch that was costing bandwidth on every valid canvas too. **Rule: a blob-URL revoke must never live in an effect with value deps.**
